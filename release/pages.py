...
COPY = {
    "en": {
        ...
        "lead": "Filing Agent investigates Korean and US company filings through follow-up questions. It runs on a personal Mac. This site replays three real investigations: an annual comparison, a company switch and an evidence refusal.",
        "story": "Why build it?",
        "story_body": "Filing Digest is the companion data project. Its latest-period digest alone cannot support historical conversation. Agent uses a separately audited, pinned fact collection derived from that foundation and original DART/EDGAR evidence. It adds conversational context, exact arithmetic, preserved results and explicit recovery. The deployed replay makes these decisions inspectable without exposing the Mac.",
        ...
        "video": "An actual new question and follow-up",
        "video_body": "Unedited browser recording of the local application: Samsung FY2023 revenue, then “What about NAVER?” in Korean. The pauses are real execution time. The interactive replay above is the more accessible route to inspect each value and source.",
    ...
    "ko": {
        ...
        "back": "녹화된 조사로 돌아가기",
        "lead": "Filing Agent는 후속 질문으로 한국·미국 기업 공시를 조사하는 로컬 앱입니다. 개인 Mac에서 실행합니다. 이 사이트에서는 실제 실행한 연간 비교, 회사 전환, 근거 부족 사례를 살펴볼 수 있습니다.",
        "story": "이 프로젝트를 만든 이유",
        ...
        "state_body": "FastAPI는 하나의 로컬 브라우저 출처를 제공합니다. LangGraph는 세 개의 실제 처리 단계를 실행하고 PostgreSQL에 체크포인트를 저장합니다. 앱 데이터에는 완료한 단계와 근거 스냅샷이 남습니다. 저장한 조사는 고정됩니다. 이어하기는 원래 근거와 맥락을 새 조사에 복사하고, 새 근거로 시작하기는 현재 승인된 스냅샷으로 새 조사를 만듭니다. 재시작 시 미완료 작업은 중단으로 표시합니다. 명시적 재시도는 한 번이며 완료한 단계를 재사용합니다. 취소는 로컬 추론 중단을 확인해야 완료됩니다.",
        "results": "측정한 동작",
        "results_body": "실행 보류한 40개 시나리오를 세 차례 측정했습니다. 매회 한국어 20개·영어 20개, 총 120회 시나리오의 기대 결과가 모두 일치했습니다. 실제 대화 턴은 138개입니다. 언어별·회차별로 정상 답변 10개와 수치 보류 또는 추가 확인 10개가 포함됩니다. 원문 수치·출처 대조에서 불일치는 없었습니다. 개발과 같은 작업 유형을 바꿔 표현한 에이전트 작성 문제이며, 임의의 금융 질문에 대한 독립 평가가 아닙니다.",
        ...
        "video": "새 질문과 후속 질문의 실제 실행",
        "video_body": "로컬 앱에서 삼성전자 2023년 매출액을 묻고 “네이버는?”으로 이어가는 무편집 브라우저 녹화입니다. 대기 시간은 실제 실행 시간입니다. 각 수치와 출처를 직접 확인하려면 대화별로 탐색할 수 있는 리플레이를 이용하세요.",
        "materials": "코드와 실행 자료",
...
def build(destination):
    ...
    for lang, c in COPY.items():
        ...
        (destination / f"engineering-{lang}.html").write_text(
            f'''<!doctype html><html lang="{lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Filing Agent · {e("title")}</title><link rel="stylesheet" href="{css}"><link rel="stylesheet" href="engineering.css"><script src="theme.js"></script></head><body><main><article class="project"><nav aria-label="Language and replay"><a href="index.html">{e("back")}</a><a lang="ko" href="engineering-ko.html">한국어</a><a lang="en" href="engineering-en.html">English</a></nav><h1>{e("title")}</h1><p class="lead">{e("lead")}</p>{sections}<section><h2>{e("video")}</h2><p>{e("video_body")}</p><video controls preload="none" aria-label="Actual local application recording"><source src="actual-run.webm" type="video/webm"></video></section><section><h2>{e("materials")}</h2><ul><li><a href="filing-agent-source.zip">{e("source")}</a></li><li><a href="LOCAL-SETUP.md">{e("setup")}</a></li><li><a href="evaluation.json">{e("evaluation")}</a></li><li><a href="evaluation-cases.json">{e("cases")}</a></li></ul><p>{e("license")}</p></section><footer>Filing Agent · Michael Ju · 2026-09-08</footer></article></main></body></html>'''
        )
